...
3a.【争点特征词】问的是某概念下的【具体争点】时（如"中止是否要求因果关系""正当防卫的时间条件"），除概念名外【必须】再给一条只有该争点才会用的特征词（如"因果""不法侵害正在进行"）——章节大词（"犯罪中止""正当防卫"）在库里命中几十处，返回窗口有限，只靠大词会把关键条目挤掉（2026-07-04 实录：中止因果关系的马工程结论排第 9 块，被"显示前 5"丢弃，答疑据此答反）。
