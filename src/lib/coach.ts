...
const RECITATION_ACTIVITY_ALIASES: Record<string, "带背" | "自背"> = { 带背: "带背", 自背: "自背" };
...
export function normalizeCoachActivity(activity: unknown): string | null {
  ...
  if (raw in RECITATION_ACTIVITY_ALIASES) return "背诵";
  return ACTIVITIES.includes(raw) ? raw : "其他";
}

export function coachRecitationMode(activity: unknown): "带背" | "自背" | null {
  if (activity == null || activity === "") return null;
...
export function withCoachRecitationModeMarker(rawInput: string, mode: "带背" | "自背" | null): string {
  ...
  return RECITATION_MODE_MARKER.test(rawInput)
    ? rawInput.replace(RECITATION_MODE_MARKER, marker)
    : `${marker} ${rawInput}`;
}
...
法制史：背多分——朝代主线+制度纵线（五刑/会审等跨朝演变）双向组织；口诀标页边后期对口诀回忆；每周至少滚3次；真题实测优先级：隋唐(每节密度第一)>清末民国>明清>秦汉。
...
/**
 * 科目方法论【详版·教练用】——比稳定前缀的 SUBJECT_METHODS 深一层：打法 + 背诵重点 + 题型打法 + 易错 + 云专属提醒。
 * 【详版·人读版】见 docs/方法论/NN_科目.md（改那边后同步这里）。仿 loadGaopinFor：只按【当轮科目】注 user 侧，
 * 不碰 system 缓存前缀、不超预算（成本账见文件头注释）。
 */
export const SUBJECT_METHOD: Record<string, string> = {
  刑法: `总则重理解（犯罪论一条线：构成→正当化→停止形态→共犯→罪数），分则按类罪名抓"构成要件+近似罪区分点"、高频优先冷门混脸熟。案例题四步：①先看设问②列"主体×行为"逐一定性③判罪数④落罪名+形态+情节（近年案例叠层3-5点）；法条分析近年考"解释能力"（术语内涵/同款不同情形/立法理由）。高频靶：因果关系(每年案例)、停止形态、共犯、罪数竞合、责任年龄、财产罪/人身罪/危害公共安全罪。★云专属：推进快≠闭环——共犯/罪数是他错题密集区，别只往分则冲、必回头做分章真题+错题复检；审题别被"构成/特别"勾走；例外边界焊死（中止自动性、法条竞合"重法优于轻法"例外）。`,
  民法: `先总后分（总则/物权通则/合同通则先扎实，越抽象越基础越要牢）。核心武器=法律行为效力"五格表"（有效/无效/可撤销/效力待定/不成立，每年6-8道，每格挂真题例）。两大薄弱核心区：物权变动（登记生效vs对抗、基于/非基于法律行为）、担保物权。法条分析五步：识考点→默框架(定义/要件/范围/例外)→定位原文→分析→分点。紧跟民法典新制度。★云专属：这是他最大空白=最大提分点（75-90分几乎没动，第2轮必全面铺开、不能再拖）；别贪快、先总后分；五格表+三抗辩权+物权变动优先做反复滚的专题卡。`,
  法理: `论述大头（骨架：概念→特征/分类→意义/联系实际）；分类辨析靠例子理解着背（真题爱在分类挖坑），框架照《考试分析》原文标题走、别套外部框架。高频靶：法的本质特征、渊源、规则三要素与分类、原则vs规则、法律关系、法律事实、价值冲突解决4原则、规范作用5种、法系(最本质=法的渊源)、法律解释、法治(习法思想/良法善治·思政区)。★云专属易错（带背已栽·进背诵易错点台账反复复检）：溯及力原则=法不溯及既往(≠新法优于旧法)、"普通法"≠"一般法"、划分标准别混、当代中国法渊源别漏行政法规；通病=答不全漏边界，答"要件/后果/分类"时自反问"还有没有另一半"。`,
  宪法: `最抽象+超纲。三件事做对就稳：①超纲法条另记（立法法/选举法/民族区域自治法/监督法/监察法）②国家机构做"职权对比表"（谁选谁/谁任免/领导vs指导/是vs不是）③数字类（任期/比例/期限/名额）单独过。高频靶：公民基本权利(最高频·各权利+限制原则如比例/法律保留)、国家机构(重灾区)、经济制度(自然资源归属)、宪法分类/修改/监督备案审查。★云专属：国家机构是真题给他标出的薄弱点——直接做对比表把"一字之差"抠死（正撞他"边界混淆"病根）；超纲四五部法条别漏；他还没启动，第1轮先搭"国家机构+公民基本权利"两大骨架(最高频)。`,
  法制史: `纯背"背多分"、命题最稳（史料引文+设问）。双线织网：朝代主线（一朝立法+司法）×制度纵线（五刑/会审等跨朝演变）；口诀标《考试分析》页边，每周至少滚3次（遗忘最快）。★频次以2014-2025十二年综合卷选择题203题实测为准（2026-07-28重算·印象版已废）：隋唐34题16.7%、清末民国34题16.7%、明清27题13.3%、秦汉25题12.3%、宋元21题10.3%、夏商西周14题、三国两晋南北朝12题(原判"高"已证伪、实为倒数第三)、革命根据地12题(十二年每年恰1题·最稳送分位)、春秋战国11题。★决定顺序的是密度不是总量：隋唐34题只压在「第四章第一节隋朝+第二节唐朝」两节里、每节产出断层第一，清末民国同样34题却摊在七八节上→第1轮从隋唐开刀。做题：选择坑=张冠李戴（谁提出/哪朝/哪部律，四要素对齐）、材料分析=看引文认制度→释义→评价。★云专属：背多分性价比高、别因"靠后突破"就一直空着；秦汉/魏晋不单开一轮、当唐律的制度渊源脚注捎带（八议入魏律、五服入晋律、北齐律12篇→开皇律12篇）；"每周滚3次"是硬指标（正撞他"背了就忘/不复盘"病根，宁可勤滚别隔太久）。`,
};
...
/**
 * 当轮科目的【方法论详版】（数百字，SUBJECT_METHOD 常量），注入 user 侧（不碰缓存前缀）。
 * 无科目轮（闲聊/谈节奏/情绪）返回空串 → 零注入零成本。人读详版见 docs/方法论/。
 */
function loadMethodFor(subject: string | null | undefined): string {
  if (!subject || !SUBJECT_METHOD[subject]) return "";
  return `【${subject}学习方法论（据此给个性化学习/背诵/做题/复盘建议·扣云的病根落地·人读详版见 docs/方法论/）】
...
【判题作答纪律——凡涉及判对错/判选项/给行为定性/辨析教义时，先守这四条，再谈风格】
云五战冲刺，一个"高信心的错误结论"比一句"我不确定"伤他十倍。所以你在判一道题、判某个说法或选项对错、给行为定性、辨析教义争点时（论策略/做规划/情绪陪伴/闲聊【不受此约束】，照常自然聊）：
1.【结论落到权威，不自由联想】每个法律结论都要能落到系统预检索给你的《考试分析》原文/做题心得/真题上（标「手动登记」的心得优先级最高）。你的直觉/印象与检索到的教材冲突时，一律以教材为准；这道点系统没检索到、或材料没讲，就【明说"教材没直接讲、我不确定"】，绝不凭印象硬编一个确定结论。
2.【先过先决关卡，再下结论】很多坑在"没判前提就直接判结论"。判主犯/从犯前先判【是否成立共同犯罪】（间接正犯利用无责任能力人、聚众犯罪只罚首要分子、事后无通谋的帮助——都不成立共犯，也就无所谓主从）；判既遂/未遂/中止前先判【是否着手、实行是否终了】；判罪数前先判【一罪还是数罪的标准】；判此罪与彼罪前先【固定各自的构成要件】。养成"先问前置概念成不成立"的习惯，别一眼扫到关键词就下结论。
3.【逐项判 + 自检一致性】判选项/多选题逐个来：定性依据 → 教材出处 → 结论。定稿前务必自查一遍——【我对某个选项用的规则，有没有在另一个选项上自己违反了？】（例：对一个选项说"不成立共犯所以不是主犯"，就绝不能对另一个同样不成立共犯的选项说"是主犯"。）自相矛盾是最丢人的错，你要自己先揪出来。
4.【拿不准就存疑、降信心】没十足把握、存在学理分歧、或可能与云手上的标答不一致时，明说"这条我不确定/建议核对标答"，把不确定摊开给云看，别用笃定口吻掩盖不确定。宁可存疑，不可误导。
...
- 心得命中里含【讲义P__·提示/马工程/规范拓展/推导逻辑/一招制敌等】的条目 =《精讲一本通》讲义（刑/民/法理/法制史四科）作者亲手划的重点、易错与权威观点，和做题心得同属①档：判题/讲考点时优先据它作答，引用时带上讲义页码（如"讲义P157·马工程"）。
- 心得命中里标「手动登记」的条目（云亲自录入）优先级最高：与其它心得或教材冲突时一律以它为准。
- 云聊某科时，该科真题高频考点会随消息给你——排优先级、考他、复盘优先从高频点下手。
- 云聊某科时，该科【学习方法论详版】也会随消息给你（打法+背诵+题型+易错+云专属提醒）——给学习/背诵/做题/复盘建议时据它落地、对症云的病根，别脱离方法论泛泛而谈。
...
${COACH_META_OPEN}
{"is_report":true/false（【只看"本轮云说"那一条消息】是否在汇报自己学了什么——他这条消息主动说"今天听了/看了/做了第几章"才是 true；提问、讨论、被你考、闲聊一律 false。⚠️此前对话里的汇报你已经记过账了，据它填 true 会造成重复记账）,"is_judging":true/false（本轮你的回复里是否做了【对错判断/选项判断/给行为定性/教义争点辨析】——判了某道题/某个说法对错、判了选项、给某行为定性就是 true；纯论策略/规划/情绪/闲聊/汇报=false。系统据此触发独立复核，据实填）,"subject":"刑法|民法|法理|宪法|法制史 或省略","chapter":"如 第3章 或省略","activity":"听课|看书|做题|背诵|自背|带背|复盘|其他 或省略","accuracy":0-100或省略,"feeling":"一句或省略","confusion":"云本轮最卡的一句或省略（仅供你理解上下文、更好追问，不入任何库，可正常填）","wrongs":["【默认空·仅主动指令】只有云本轮明确说'记进错题本/记录进错题本/这个记错题'时才填对应考点"],"xinde_notes":["【默认空·仅主动指令】只有云本轮明确说'记录进心得/记进心得/这条记下来'时，才把那条规律原话整理成一句填进来"],"absorbed":["今天云明确说懂了/掌握了的考点短语"],"log_fix":{"subject":"刑法","from":"原记录里的章节关键词（如'第四章'）","to":"修正后的章节全名"}（【默认省略·仅主动指令】只有云本轮明确要求修正/改正之前记错的进度时才填）,"memory_updates":[{"fact":"关于云的【新】耐久事实","category":"画像|倾向|目标|偏好|约束"}]}
${COACH_META_CLOSE}
...
- **背诵活动统一**：用户自己背完填 activity="自背"，系统实际带背收官填 activity="带背"；写入层都归一为 activity="背诵"，但会在原话中保留方式标记。只写 activity="背诵" 会失去方式信息，应尽量避免。
...
async function planAndSearch(input: string, conversationTail: string[], signal?: AbortSignal, image?: ChatImage | null): Promise<{ block: string; costUsd: number; subject: string | null }> {
  const planSystem = `你是法硕教练的检索规划器。教练回答前可先查库：search_textbook（《考试分析》教材+刑/民/法理/法制史四科讲义原文）/ search_xinde（做题心得 + 四科讲义心得：作者标注的提示/马工程/规范拓展/推导逻辑/一招制敌等框，逐字提取，是作者划的重点与权威观点）/ search_zhenti（真题，参数 year、question_no）。
两件事：
...
export async function runCoach(
  ...
  const methodBlock = loadMethodFor(planSubject);
  const extraBlocks = [methodBlock, gaopinBlock, grepBlock].filter(Boolean).join("\n\n");
  ...
  const parsed = {
    ...
    recitationMode: coachRecitationMode(meta?.activity),
    ...
    if (dup) {
      ...
      const { data: logRow, error: logErr } = await supabaseAdmin
        ...
        .insert({
          ...
          source: "manual",
          raw_input: withCoachRecitationModeMarker(input, parsed.recitationMode),
        })
        .select("id")
        .single();
