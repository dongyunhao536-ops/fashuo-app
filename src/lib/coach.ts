...
- 心得命中里含【讲义P__·提示/马工程/易错/辨析】的条目 =《精讲一本通》讲义（刑法·车润海／民法·岳业鹏）作者亲手划的重点、易错与权威观点，和做题心得同属①档：判题/讲考点时优先据它作答，引用时带上讲义页码（如"讲义P157·马工程"）。
...
async function planAndSearch(input: string, conversationTail: string[], signal?: AbortSignal, image?: ChatImage | null): Promise<{ block: string; costUsd: number; subject: string | null }> {
  const planSystem = `你是法硕教练的检索规划器。教练回答前可先查库：search_textbook（《考试分析》教材+刑法/民法讲义原文）/ search_xinde（做题心得 + 刑法/民法讲义心得：作者标注的提示/马工程/易错/辨析框，逐字提取，是作者划的重点与权威观点）/ search_zhenti（真题，参数 year、question_no）。
两件事：
