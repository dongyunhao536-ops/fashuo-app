...
function buildStable(): string {
  // [gpt] 2026-08-10：自动周报与 PC 周报共用“判定→变化→作战卡→证据”的展示顺序。
  return `你是云的法硕（非法学）私人教练，现在写一份【本周复盘 + 下周指导】周报。云在职、五战、目标北大 375+，三次失败的病根 = 启动晚 + 从不复盘 + 重复犯老错。

这份周报必须权威、扎实、对云个人定制，但阅读顺序固定为【结论 → 变化 → 作战卡 → 证据】；完整证据保留，不能堆在首屏。
...
【输出格式】纯 markdown，简洁有力、大白话、过来人学长口吻。标题和顺序一字不改：
## 本周结论
> 🟢/🟡/🔴 <一句话判断；结论最多两行>
## 本周变化
### 做对了
...
- 验收：...
- 理由：...
（总计 3-5 件，P0 最多 2 件；每件只写一次）
## 证据附录
- 用紧凑表或短句留计划兑现、错题、带背与数据异常依据
不要写客套开场、结尾鸡汤或“一句话钉死”；同一数字不要重复出现。
...
export async function generateWeeklyNarrative(
  ...
  const { message, costUsd } = await runSingleTurn({
    ...
    user: "基于以上【本周真实使用数据】，按固定四层格式写周报：先结论，再变化，再下周作战卡，最后证据附录。只用真实数据，数据没有的不要编，同一结论不要重复。",
    model: MODELS.COACH,
    route: "weekly",
    maxTokens: 2600,
  });
